...
export class EmployeePage extends BasePage {
    ...
    async search(name: string) {
        const searchNameInput = this.page
        ...
        // Wait for hint dropdown and click the option to properly register the search filter
        try {
            const option = this.page.locator('[role="listbox"] .oxd-autocomplete-option').first();
            await option.waitFor({ state: 'visible', timeout: 5000 });
            await option.click();
        } catch {
            console.log(`[EmployeePage.search] No hint dropdown appeared for ${name}`);
        }
    ...
    async create(firstName: string, lastName: string): Promise<string> {
        await this.page.goto(
            `${process.env.BASE_URL}/web/index.php/pim/addEmployee`,
        );
        ...
        console.log(
            `[EmployeePage.create] Filling form for ${firstName} ${lastName}`,
        );
        ...
        const idInput = this.getInputByLabel(/Employee Id/i);
        ...
        try {
            ...
            console.error(
                "[EmployeePage.create] Error: Navigation to personalDetails timed out.",
            );
            ...
            console.error(
                `[EmployeePage.create] Diagnostic Error Check: ${errorMsg}`,
            );
            throw e;
        }
        ...
        try {
            ...
            console.log(
                "[EmployeePage.create] Warning: Could not extract ID, returning empty string.",
            );
            return "";
        }
    }
    ...
    async read(firstName: string): Promise<boolean> {
        ...
        try {
            console.log(
                `[EmployeePage.read] Waiting for employee row: ${firstName}`,
            );
            await this.waitForTableRowVisible(firstName, 20000);
            return true;
        } catch {
    ...
    async update(firstName: string, newLastName: string): Promise<void> {
        ...
        const row = this.getTableRowByText(firstName);
        ...
        // Wait for success toast and let it settle
        await this.waitForToastSuccess(25000);
    }
