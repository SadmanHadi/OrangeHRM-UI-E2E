 ...
 */
export async function deleteClaimAndEvent(page: Page, eventName: string): Promise<void> {
  const claimPage = new ClaimPage(page);
  // Attempt claim deletion (might be disabled by UI limitations)
  await claimPage
    .delete(eventName)
    .catch((err) => console.warn(`Claim cleanup failed (expected for initiated claims): ${err}`));
